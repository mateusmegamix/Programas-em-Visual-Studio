...
namespace SICOP.VIEW
{
    public partial class frmRegistrarES : Form
    {
        ...
        List<Carreta> mainListaCarreta = new List<Carreta>();
        int indexListaCarreta = 0;
        ...
        private bool VerificarSaida()
        {
            bool retorno = false;

            List<Saida> listaS = CSaida.CarregarPK((int)dtgPessoa.CurrentRow.Cells[0].Value);
            List<Entrada> listaE = CEntrada.CarregarPK((int)dtgPessoa.CurrentRow.Cells[0].Value);

            if (listaE.Count > listaS.Count)
            {
                retorno = true;
            }
        ...
        {
            btnSaida.Enabled = VerificarSaida();
            ...
            if (btnSaida.Enabled)
            {
                ...
                mainListaCarreta.Clear();
                indexListaCarreta = 0;
                List<Carreta> temp = CCarreta.CarregarCarreta();
                for(int i = 0; i < temp.Count; i++)
                {
                    if (temp[i].CarSaiCodigo == 0)
                    {
                        mainListaCarreta.Add(temp[i]);
                    }
                }

                if(mainListaCarreta.Count > 0)
                {
                    txtTipo.Text = mainListaCarreta[0].CarTipo;
                    txtPlaca.Text = mainListaCarreta[0].CarPlaca;
                    txtCidade.Text = mainListaCarreta[0].CarCidade;

                    if(mainListaCarreta.Count > 1)
                    {
                        btnProCar.Enabled = true;
                    }
                }
                else
                {
                    cbxCarreta.SelectedIndex = 1;
                    cbxCarreta.Enabled = false;                  
                }
            }
            else
            {
                ...
                btnProCar.Enabled = false;
                btnAntCar.Enabled = false;
            }
        }
        ...
        private void cbxCarreta_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbxCarreta.SelectedIndex == 0)
            {
                gbxCarreta.Enabled = true;
            }
            else
            {
                gbxCarreta.Enabled = false;
            }
        ...
        private void btnEntrada_Click(object sender, EventArgs e)
        {
            if (dtgPessoa.RowCount > 0)
            {
                ...
                if (pessoa != null)
                {
                    if ((cbxCarreta.SelectedIndex == 0 && txtTipo.Text != string.Empty && txtPlaca.Text != string.Empty
                       && txtCidade.Text != string.Empty) || cbxCarreta.SelectedIndex == 1)
                    {
                        try
                        {
                            Entrada entrada = new Entrada();
                            entrada.EntPesCodigo = pessoa.PesCodigo;
                            entrada.EntData = DateTime.Now.ToShortDateString();
                            entrada.EntHora = DateTime.Now.ToShortTimeString();
                            bool registroFoiIncluido = CEntrada.Incluir(entrada);
                            if (registroFoiIncluido == true && cbxCarreta.SelectedIndex == 0)
                            {
                                Carreta carreta = new Carreta();
                                carreta.CarEntCodigo = CEntrada.ObterCodigo();
                                carreta.CarTipo = txtTipo.Text;
                                carreta.CarPlaca = txtPlaca.Text;
                                carreta.CarCidade = txtCidade.Text;

                                CCarreta.Incluir(carreta);
                            }
                        }
                        catch (Exception ex)
                        {
                ...
                if (pessoa != null)
                {
                       ...
                       && txtCidade.Text != string.Empty) || cbxCarreta.SelectedIndex == 1)
                    {
                        try
                        {
                            ...
                            saida.SaiPesCodigo = pessoa.PesCodigo;
                            saida.SaiData = DateTime.Now.ToShortDateString();
                            saida.SaiHora = DateTime.Now.ToShortTimeString();
                            bool registroFoiIncluido = CSaida.Incluir(saida);
                            if (registroFoiIncluido == true && cbxCarreta.SelectedIndex == 0)
                            {
                                Carreta carreta = mainListaCarreta[indexListaCarreta];
                                carreta.CarSaiCodigo = CSaida.ObterCodigo();
                                CCarreta.Alterar(carreta);
                            }
        ...
        private void btnProCar_Click(object sender, EventArgs e)
        {
            if (indexListaCarreta < mainListaCarreta.Count - 1)
            {
                indexListaCarreta++;

                if(indexListaCarreta > 0)
                {
                    btnAntCar.Enabled = true;
                }

                txtTipo.Text = mainListaCarreta[indexListaCarreta].CarTipo;
                txtPlaca.Text = mainListaCarreta[indexListaCarreta].CarPlaca;
                txtCidade.Text = mainListaCarreta[indexListaCarreta].CarCidade;
            }
        ...
        private void btnAntCar_Click(object sender, EventArgs e)
        {
            if (indexListaCarreta > 0 && indexListaCarreta <= mainListaCarreta.Count - 1)
            {
                indexListaCarreta--;
